...
class AuthManager {
    ...
    handleRegistration(form) {
        return new Promise((resolve, reject) => {
            ...
            // Clear all storage and initialize users array if no users exist
            const existingUsers = this.getStoredUsers();
            if (!Array.isArray(existingUsers) || existingUsers.length === 0) {
                localStorage.clear(); // Clear all storage for fresh start
            ...
            setTimeout(() => {
                ...
                if (result.success) {
                    ...
                    this.updateUIBasedOnAuth();
                    this.closeAuthModal(); // Close the registration modal
    ...
    createUser(userData) {
        ...
        // For first admin registration, ensure clean state
        if (!Array.isArray(users) || users.length === 0) {
            localStorage.clear(); // Clear all storage for fresh start
            localStorage.setItem('registeredUsers', '[]');
            users = [];
        } else if (users.find(u => u.email === userData.email)) {
            // Check if user already exists (only after first admin)
